...
class ListTissuesSeeder extends Seeder
{
    ...
    {
        $target_table_name = 'list_tissues';
        DB::table($target_table_name)->truncate();

        $now = Carbon::now();
        $path = base_path().'/database/seeders/seeds/literature/list_tissues.csv';
        ...
        $this->command->info('Seeding list_tissues table...');
        ...
        foreach ($rows as $r) {
            ...
            // Clean the value
            $cleanedValue = $this->cleanValue($r['tissue']);
...
// php artisan db:seed --class=Database\\Seeders\\Literature\\ListTissuesSeeder
