...
class ListUseCategoriesSeeder extends Seeder
{
    ...
    {
        $target_table_name = 'list_use_categories';
        DB::table($target_table_name)->truncate();

        $now = Carbon::now();
        $path = base_path().'/database/seeders/seeds/literature/list_use_categories.csv';
        ...
        $this->command->info('Seeding list_use_categories table...');
        ...
        foreach ($rows as $r) {
            // Skip empty rows
            if (empty($r['use_category'])) {
                continue;
            }

            // Clean the value
            $cleanedValue = $this->cleanValue($r['use_category']);
...
// php artisan db:seed --class=Database\\Seeders\\Literature\\ListUseCategoriesSeeder
