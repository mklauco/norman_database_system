...
class ARBGGeneDataSourceSeeder extends Seeder
{
    ...
    {
        $this->command->info('Starting ARBG Gene data source main seeding...');
        $target_table_name = $this->table_prefix.'gene_data_source';
        $now = Carbon::now();
        $startTime = microtime(true);
        $path = base_path().'/database/seeders/seeds/'.$this->table_prefix.'tables/arg_data_source.csv';
        ...
        $this->command->info('ARBG Gene data source main seeding completed!');
    }
}
// php artisan db:seed --class=ARBGGeneDataSourceSeeder
