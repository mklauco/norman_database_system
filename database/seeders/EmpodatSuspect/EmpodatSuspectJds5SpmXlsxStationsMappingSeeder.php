...
/**
 * Insert one row per station column from JDS5 SPM into
 ...
 *
 * Source file: storage/app/public/empodat_suspect/SUSPECT_SPM_JDS5_EI_20260825.xlsx
 * Station columns: everything AFTER 'Units' and BEFORE 'mz score', MINUS control
 * columns. 9 columns in that block, of which two are controls — `Fabric Blank
 * for SPM JDS5` and `Procedural Blank for SPM JDS5` — leaving 7 stations.
 *
 * The control test is `contains "blank"`, which is what catches both of these;
 * the TerraChem `starts with "Blank"` rule would catch neither.
 */
class EmpodatSuspectJds5SpmXlsxStationsMappingSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command->info('Reading xlsx headers from JDS5 SPM (file_id='.self::FILE_ID.')...');
...
// php artisan db:seed --class=Database\\Seeders\\EmpodatSuspect\\EmpodatSuspectJds5SpmXlsxStationsMappingSeeder
