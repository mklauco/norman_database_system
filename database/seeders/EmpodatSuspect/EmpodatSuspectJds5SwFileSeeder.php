...
/**
 * Register the JDS5 SURFACE WATER source file (id=10020) in the `files` table, then
 * write its provenance row in `empodat_suspect_data_source`.
 *
 * One-file FileSeeder — scoped to the JDS5 SURFACE WATER pipeline only. Called by
 * EmpodatSuspectJds5SwSeeder so the orchestrator doesn't touch unrelated
 * file rows.
 *
 ...
 * the case everywhere for 10020 — would be created on the broken files_id_seq
 ...
 * ever revisited.
 */
class EmpodatSuspectJds5SwFileSeeder extends Seeder
{
    use WithoutModelEvents;

    private const int FILE_ID = 10020;
...
// php artisan db:seed --class=Database\\Seeders\\EmpodatSuspect\\EmpodatSuspectJds5SwFileSeeder
