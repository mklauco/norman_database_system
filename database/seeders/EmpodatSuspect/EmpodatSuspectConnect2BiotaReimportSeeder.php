...
/**
 * CONNECT 2 BIOTA re-import pipeline (file_id=10004), end to end.
 *
 * Phases:
 *   0. Scoped teardown  — delete ONLY file_id=10004: substances, metadata,
 ...
 *                         8 resolve to exactly one station each.
 *   4. Main + metadata  — stream the spreadsheet back into the SAME id block
 *                         (6414447..7180350), in one transaction.
 *
 ...
 * refreshed.
 *
 * php artisan db:seed --class=Database\\Seeders\\EmpodatSuspect\\EmpodatSuspectConnect2BiotaReimportSeeder
 */
class EmpodatSuspectConnect2BiotaReimportSeeder extends Seeder
{
    use DeletesSuspectFileData;
    use WithoutModelEvents;

    private const int FILE_ID = 10004;

    private const int ID_FROM = 6414447;

    private const int ID_TO = 7180350;
    ...
    {
        $this->command->warn('=== CONNECT 2 BIOTA re-import (file_id='.self::FILE_ID.') ===');
        ...
        $this->call([
            EmpodatSuspectConnect2BiotaFileSeeder::class,
        ...
        $this->callWith(EmpodatSuspectConnect2BiotaMainSeeder::class, ['useFixedIdRange' => true]);

        $this->command->info('=== CONNECT 2 BIOTA re-import complete ===');
