...
/**
 * Register the JDS5 SPM source file (id=10019) in the `files` table, then
 * write its provenance row in `empodat_suspect_data_source`.
 *
 * One-file FileSeeder — scoped to the JDS5 SPM pipeline only. Called by
 * EmpodatSuspectJds5SpmSeeder so the orchestrator doesn't touch unrelated
 * file rows.
 *
 ...
 * the case everywhere for 10019 — would be created on the broken files_id_seq
 ...
 * ever revisited.
 */
class EmpodatSuspectJds5SpmFileSeeder extends Seeder
{
    use WithoutModelEvents;

    private const int FILE_ID = 10019;
    ...
    public function run(): void
    {
        ...
        $file->forceFill([
            'id' => self::FILE_ID,
            'original_name' => 'SUSPECT_SPM_JDS5_EI_20260825.xlsx',
            'name' => 'JDS5 SPM Suspect Screening Results',
            'description' => 'Joint Danube Survey 5 — suspect screening, suspended particulate matter samples, analysed by the Environmental Institute. Includes HRMS identification metadata stored in empodat_suspect_metadata.',
            'file_path' => 'empodat_suspect/SUSPECT_SPM_JDS5_EI_20260825.xlsx',
...
// php artisan db:seed --class=Database\\Seeders\\EmpodatSuspect\\EmpodatSuspectJds5SpmFileSeeder
