...
/**
 * Insert one row per station column from JDS5 GROUNDWATER into
 * empodat_suspect_xlsx_stations_mapping (with station_id NULL — the Fill
 * seeder resolves it via LOWER(sample_code) = LOWER(xlsx_name)).
 *
 * Source file: storage/app/public/empodat_suspect/SUSPECT_GW_JDS5_EI_20260627.xlsx
 * Station columns: everything AFTER 'Units' and BEFORE 'mz score' (the metadata
 * block boundary), MINUS any control column. 8 columns in that block, of which
 * `Blank GW JDS5` is the control, leaving 7 stations: GW1..GW7_GRB:EISK.
 *
 * The control-column test is `contains "blank"`, inherited from the JDS5 BIOTA
 * seeder. This file happens to prefix its control column the TerraChem way, so
 * either rule would work.
 */
class EmpodatSuspectJds5GwXlsxStationsMappingSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command->info('Reading xlsx headers from JDS5 GROUNDWATER (file_id='.self::FILE_ID.')...');
...
// php artisan db:seed --class=Database\\Seeders\\EmpodatSuspect\\EmpodatSuspectJds5GwXlsxStationsMappingSeeder
