...
/**
 * Register the TerraChem SOIL 2nd batch source file (id=10021) in the `files` table, then
 * write its provenance row in `empodat_suspect_data_source`.
 *
 * One-file FileSeeder — scoped to the TerraChem SOIL 2nd batch pipeline only. Called by
 * EmpodatSuspectTerraChemSoilBatch2Seeder so the orchestrator doesn't touch unrelated
 * file rows.
 *
 ...
 * the case everywhere for 10021 — would be created on the broken files_id_seq
 ...
 * ever revisited.
 */
class EmpodatSuspectTerraChemSoilBatch2FileSeeder extends Seeder
{
    use WithoutModelEvents;

    private const int FILE_ID = 10021;
    ...
    public function run(): void
    {
        ...
        $file->forceFill([
            'id' => self::FILE_ID,
            'original_name' => 'TerraChem Soil 2nd Batch.xlsx',
            'name' => 'TerraChem SOIL 2nd Batch Suspect Screening Results',
            'description' => 'TerraChem — suspect screening, SOIL samples, 2nd batch. Includes HRMS identification metadata stored in empodat_suspect_metadata.',
            'file_path' => 'empodat_suspect/TerraChem Soil 2nd Batch.xlsx',
...
// php artisan db:seed --class=Database\\Seeders\\EmpodatSuspect\\EmpodatSuspectTerraChemSoilBatch2FileSeeder
