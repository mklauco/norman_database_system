...
/**
 * Stream TerraChem SOIL 2nd batch xlsx → populate `empodat_suspect_main`,
 ...
 *     This particular file carries none; the skip is kept because other
 *     TerraChem files do.
 *   - HRMS column is spelled `numberoffragments score` (with "ber"), not
 *     `numoffragments score` — reflected in METADATA_COLUMN_MAP below.
 *
 * TerraChem SOIL 2nd batch notes:
 *   - 17 station columns, no control column at all — unusual for TerraChem,
 *     whose other sources carry one or more Blank columns.
 *   - Standard Block A including `Units`, so STATION_BLOCK_START_AFTER stays
 *     `Units` (contrast the JDS5 surface water file, 10020, which has no Units
 *     column at all).
 *   - The metadata block is complete: all 13 columns present.
 *   - Units are `μg/kg dw`, a solid matrix.
 *
 * Inherited notes from the TerraChem lineage:
 *   - 58 station columns, not 83, and their headers are the full
 *     `TChem-…-2025` form matched against empodat_stations.sample_code — see
 *     {@see EmpodatSuspectTerraChemSoilBatch2XlsxStationsMappingFillSeeder}.
 *     The two files' header sets do not intersect, which matters because
 *     {@see LoadsSubstanceCaches::loadStationMappingCache()} keys the mapping
 *     cache by xlsx_name across every file_id, not per file.
 *   - Column block boundaries and the whole HRMS metadata block are identical
 *     to 10012 (verified against the delivered file, 2026-09-05).
 *
 * See: Empodat-Suspect-3-new-source.md §11 (TerraChem)
 */
class EmpodatSuspectTerraChemSoilBatch2MainSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command->info('Streaming TerraChem SOIL 2nd batch → empodat_suspect_main + empodat_suspect_metadata + empodat_suspect_substances (file_id='
            .self::FILE_ID.')...');
...
// php artisan db:seed --class=Database\\Seeders\\EmpodatSuspect\\EmpodatSuspectTerraChemSoilBatch2MainSeeder
