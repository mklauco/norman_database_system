...
/**
 * Register the JDS5 GROUNDWATER source file (id=10018) in the `files` table, then
 * write its provenance row in `empodat_suspect_data_source`.
 *
 * One-file FileSeeder — scoped to the JDS5 GROUNDWATER pipeline only. Called by
 * EmpodatSuspectJds5GwSeeder so the orchestrator doesn't touch unrelated
 * file rows.
 *
 ...
 * the case everywhere for 10018 — would be created on the broken files_id_seq
 ...
 * ever revisited.
 */
class EmpodatSuspectJds5GwFileSeeder extends Seeder
{
    use WithoutModelEvents;

    private const int FILE_ID = 10018;
    ...
    public function run(): void
    {
        ...
        $file->forceFill([
            'id' => self::FILE_ID,
            'original_name' => 'SUSPECT_GW_JDS5_EI_20260627.xlsx',
            'name' => 'JDS5 GROUNDWATER Suspect Screening Results',
            'description' => 'Joint Danube Survey 5 — suspect screening, groundwater samples, analysed by the Environmental Institute. Includes HRMS identification metadata stored in empodat_suspect_metadata.',
            'file_path' => 'empodat_suspect/SUSPECT_GW_JDS5_EI_20260627.xlsx',
...
// php artisan db:seed --class=Database\\Seeders\\EmpodatSuspect\\EmpodatSuspectJds5GwFileSeeder
